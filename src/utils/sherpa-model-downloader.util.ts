...
async function ensureAsr(): Promise<void> {
    ...
    if (process.env.SHERPA_ONNX_ASR_ENCODER_PATH && process.env.SHERPA_ONNX_ASR_DECODER_PATH) {
        logger.info('sherpa-onnx ASR: using env-configured models');
        return;
    }

    if (fs.existsSync(encoderPath) && fs.existsSync(decoderPath)) {
        setAsrEnv(encoderPath, decoderPath);
...
async function ensureTts(): Promise<void> {
    ...
    // If user supplied env vars manually, trust them
    if (process.env.SHERPA_ONNX_TTS_MODEL_PATH && process.env.SHERPA_ONNX_TTS_TOKENS_PATH) {
...
function setAsrEnv(encoder: string, decoder: string): void {
